/*
 * @Author: Wang Zongwu
 * @Date: 2022-03-23 00:00:47
 * @LastEditTime: 2022-03-23 01:44:15
 * @LastEditors: Wang Zongwu
 * @Description: Implementation of LRU cache replacement policy. 
 * @reference: 
 * https://www.cnblogs.com/wanghao-boke/p/12162157.html
 * https://blog.csdn.net/Stand1210/article/details/70175792
 * @FilePath: /tmp/home/wzw/Documents/ACA_Proj/Tools/ChampSim/inc/lru_cache.h
...
int LRUCacheSet(void *lruCache, size_t key, size_t data);

size_t LRUCacheGet(void *lruCache, size_t key);
...
//创建一个缓存单位
static cacheEntryS *newCacheEntry(size_t key, size_t data)
{
...
//哈希函数
static int hashKey(LRUCacheS *cache, size_t key)
{
    return (int)key%cache->cacheCapacity;
}

//从哈希表中获取缓存单元
static cacheEntryS *getValueFromHashMap(LRUCacheS *cache, size_t key)
{
    //使用函数定位数据存放在哪个槽中
